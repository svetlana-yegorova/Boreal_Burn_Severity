...
# with geomorphons
# bs_df<-readRDS("./Documents/Boreal_Burn_Severity/outputs/full_bs_gmf.RDS")

# with ecozones: 
bs_df<-readRDS("./Documents/Boreal_Burn_Severity/outputs/full_bs&e.RDS")

# ecozone names: 
ecz_tbl<-read.csv("./Documents/Boreal_Burn_Severity/outputs/ecozone_names&codes.csv")
...
#### what is happening with the ecoregion data? ####
ecoz_na<-bs_df_clean|>
  filter(is.na(ECOZONE))

# add ecozone name to the table: 
bs_df_clean<-left_join(bs_df_clean, ecz_name)
...
bs_df_clean|>
  ...
  # slice_sample(prop=0.01) |>
  filter(severity_1>1 & severity_2>0)|>
  ggplot()+
  ...
  facet_wrap(~as.factor(ZONE_NAME), scales="free_y")+
...
ggsave("./Documents/Boreal_Burn_Severity/outputs/visuals/1_to_2nd_severity_change_overall_with_ecozone.png",
...
bs_df_clean|>
  mutate(sev_1f=factor(severity_1, 
                       ...
                       labels=c("unburned", "low", "moderate", "high") ))|>
  # slice_sample(prop=0.01) |>
  filter(severity_1>1 & severity_2>1&ECOZONE %in% c(4, 5))|>
  ggplot()+
  ...
  # labs(fill="severity of \n1st fire")+
  # theme(legend.title = element_blank() )+ 
  facet_wrap(as.factor(ZONE_NAME) ~ as.factor(interval_name), scales = "free_y")+
  # theme_void()+
...
### severity by geomorphon type ###
# 1. What is the severity distribution at different geomorphic positions? 
# 2. What is the interval distribution at diff geomorphic positions? 
# 3. What is the relationship between 1st and 2nd severity at diff geomorphic positions? 
...
bs_df_clean|>
  ...
  mutate(sev_1f=factor(severity_1,
                       ...
                       labels=c("unburned", "low", "moderate", "high") ))|>
  ggplot()+
  geom_histogram(aes(x=severity_1, fill=as.factor(gmf)), position="dodge", binwidth = 1)+
  facet_wrap(~as.factor(gmf)) +
  scale_fill_manual(values = c(
    "low"="lightgreen",
    "moderate" = "lightblue",  "high" = "pink2"))+
   theme_classic()
